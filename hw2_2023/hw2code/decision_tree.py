...
def compute_info_gain(data, att_name, y_name):
    ...
    #========================#
    curr_entropy = compute_entropy(data[y_name])
    entropy = np.sum([(counts[i]/total_counts)*compute_entropy(data.where(data[att_name]==vals[i]).dropna()[y_name]) for i in range(len(vals))])
    
    info_gain = curr_entropy - entropy
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    # START YOUR CODE HERE  #
    #========================#
    att_info = -1 * np.sum([(counts[i]/total_counts)*np.log2(counts[i]/total_counts) for i in range(len(vals))])
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
...
class DecisionTree(object):
    ...
    def make_tree(self, train_data, parent_node_class = None):
        ...
        if len(np.unique(data[self.y_name])) <= 1:
            ...
            # START YOUR CODE HERE  #
            #========================#
            leaf_value = np.unique(data[self.y_name])[0]
            #========================#
            #   END YOUR CODE HERE   #
            #========================# 
            return leaf_value
